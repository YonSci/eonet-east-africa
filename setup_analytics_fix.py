...
function exportGeoJSON(events) {
  const fc = {
    ...
    features: events.filter((ev) => ev.coords).map((ev) => ({
      type: 'Feature',
...
function exportCSV(events) {
  const header = ['ID','Title','Category','Status','Date','Closed',
                  'Magnitude','Unit','Country','Lat','Lon']
...
function DurationChart({ events }) {
  const data = useMemo(() => {
    ...
    events.filter((ev) => ev.status === 'closed').forEach((ev) => {
...
function CountryTable({ events }) {
  ...
  const maxTotal = rows[0] ? rows[0].total : 1
...
function SourceList({ events }) {
  ...
  const max = sources[0] ? sources[0].count : 1
...
export default function AnalyticsPanel() {
  ...
  // Apply same filters as map and event list
  ...
  return (
    <div style={{
      ...
      <Card title="Events by category (click bar to filter map)"
        style={{ gridColumn:'1 / 2' }}>
        <CategoryChart events={events} />
      </Card>

      {/* Status donut + sources */}
      ...
      {/* Seasonal pattern heatmap -- events in scope here */}
      <Card title="Seasonal event patterns (month x category)"
      ...
      <Card title="Export data" style={{ gridColumn:'1 / -1' }}>
        ...
        <p style={{ fontSize:11, color:'var(--text-muted)', marginTop:10 }}>
          Exports apply your current filters. GeoJSON is compatible with
          QGIS, ArcGIS, and Python geopandas.
        </p>
      </Card>

      {/* Year-over-year comparison -- events in scope, YearComparison fetches its own data */}
